...
async function readQuestionData(page) {
    return page.evaluate((enableDiscussionFallback) => {
        ...
        for (const s of analysisSelectors) {
            ...
            if (elements.length > 0) console.log(`[Selector Match] ${s}: ${elements.length} found`);
            for (let i = elements.length - 1; i >= 0; i--) {
                const el = elements[i];
                if (isVisible(el)) {
                    const t = el.innerText.trim();
                    console.log(`[Analysis Candidate] From ${s}: Length ${t.length}, Visible: ${isVisible(el)}`);
...
async function crawlSubject(page, subject) {
    ...
    for (const cat of CATEGORIES) {
        ...
        for (const chapter of chapters) {
            if (!chapter.title.includes('通用模式')) continue;
            // 核心修复：直接使用固定的 subject.name 作为 Key
，防止 DOM 读取不稳定导致进度记录失败
            ...
            if (false && (savedInfo.isFinished || (totalGoal > 0 && currentProgress >= totalGoal))) {
            ...
            try {
                ...
                // 强制从第1题开始用于测试
                if (true) {
                    fs.writeFileSync(outputFile, `# ${chapter.title} (TEST RUN)\n\n`);
                    currentProgress = 0;
                }

                let lastWrittenCurrLocal = currentProgress; 
                let lastFingerprint = ''; // 数据指纹，防止重复读取旧内容
                let lastStem = '';        // 用于共享题干去重
                let stuckCount = 0;       // 页面卡死计数器
                let retryCount = 0;       // 重复题号计数器
                let testCount = 0;        // 测试计数
...
async function run() {
    ...
    log(`\n===========================================================`, 'INFO');
    log(`[测试模式] 仅处理 1 套题库，开始全自动遍历...`, 'INFO');
    log(`===========================================================\n`, 'INFO');
