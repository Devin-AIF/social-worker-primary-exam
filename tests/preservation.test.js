...
test('Shared-case: isProbablyStale returns false when title fingerprints match (same parent question)', () => {
    // The prefix must be > 50 chars so that substring(0,50) of both fingerprints is identical
    const sharedTitlePrefix = '某社会工作服务机构计划运用小组工作法开展服务，以下是相关情况，请根据材料回答以下问题，该机构共有五个问题需要回答';
...
test('FIFO: pushing 4 entries keeps pool at max 3 (unfixed push logic)', () => {
    const pool = [];
    const entries = [
        '未知|参考答案：第一题解析内容足够长以通过30字符阈值检查，这里补充更多内容确保超过阈值',
        '未知|参考答案：第二题解析内容足够长以通过30字符阈值检查，这里补充更多内容确保超过阈值',
        '未知|参考答案：第三题解析内容足够长以通过30字符阈值检查，这里补充更多内容确保超过阈值',
        '未知|参考答案：第四题解析内容足够长以通过30字符阈值检查，这里补充更多内容确保超过阈值'
...
test('FIFO: duplicate entries are NOT added twice', () => {
    const pool = [];
    const entry = '未知|参考答案：第一题解析内容足够长以通过30字符阈值检查，这里补充更多内容确保超过阈值'.replace(/\s/g, '').substring(0, 160);
    pushToStalePool_unfixed(pool, entry);
...
test('Non-matching: isProbablyStale returns false when fingerprint is not in pool', () => {
    const poolEntry = '未知|参考答案：第一题解析内容足够长以通过30字符阈值检查，这里补充更多内容确保超过阈值'.replace(/\s/g, '').substring(0, 160);
    const differentFingerprint = '参考答案：第二题完全不同的解析内容与第一题没有任何关联，这里补充更多内容确保超过阈值'.replace(/\s/g, '').substring(0, 100);
...
test('Exact match: isProbablyStale returns true when fingerprint exactly matches pool entry', () => {
    const fingerprint = '参考答案：第一题解析内容足够长以通过30字符阈值检查并且超过50字符以触发深度检查逻辑，这里补充更多内容'.replace(/\s/g, '');
...
test('Substring match: isProbablyStale returns true when fingerprint contains pool entry as substring', () => {
    ...
    assert.ok(longFingerprint.length > 50, `Prerequisite: fingerprint > 50 chars (got ${longFingerprint.length})`);
    assert.ok(shortPoolEntry.length > 50, `Prerequisite: pool entry > 50 chars (got ${shortPoolEntry.length})`);
